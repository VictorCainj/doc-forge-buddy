    ...
    if (state?.editMode && state?.analiseData && contracts.length > 0) {
      ...
      if (state.analiseData?.contract_id) {
        const contract = contracts.find(
          (c) => c.id === state.analiseData?.contract_id
        );
        if (contract) {
  ...
  useEffect(() => {
    if (selectedContract) {
      setDadosVistoria({
        ...
        endereco:
          selectedContract.form_data.enderecoImovel ||
  ...
  const saveAnalise = useCallback(async () => {
    if (!selectedContract || apontamentos.length === 0) {
      toast({
        ...
        description:
          'Selecione um contrato e adicione pelo menos um apontamento',
        variant: 'destructive',
      });
      return;
    }
