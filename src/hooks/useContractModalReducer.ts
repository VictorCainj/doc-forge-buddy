...
export type ContractModalAction =
  ...
  | {
      type: 'OPEN_WHATSAPP_MODAL';
      payload: { contract: Contract; type: PersonType };
    }
  | { type: 'CLOSE_WHATSAPP_MODAL' }
  | {
      type: 'OPEN_ASSINANTE_MODAL';
      payload: { contract: Contract; template: string; documentType: string };
    }
  ...
  // Ações de formulário
  | {
      type: 'UPDATE_AGENDAMENTO_DATA';
      payload: Partial<ContractModalState['formData']['agendamento']>;
    }
  | {
      type: 'UPDATE_RECUSA_DATA';
      payload: Partial<ContractModalState['formData']['recusaAssinatura']>;
    }
  | {
      type: 'UPDATE_WHATSAPP_DATA';
      payload: Partial<ContractModalState['formData']['whatsApp']>;
    }
  | {
      type: 'UPDATE_ASSINANTE_DATA';
      payload: Partial<ContractModalState['formData']['assinante']>;
    }
  | {
      type: 'UPDATE_NPS_DATA';
      payload: Partial<ContractModalState['formData']['nps']>;
    }
    ...
    case 'UPDATE_RECUSA_DATA':
      return {
        ...
        formData: {
          ...state.formData,
          recusaAssinatura: {
            ...state.formData.recusaAssinatura,
            ...action.payload,
          },
        },
      };
...
export const useContractModalReducer = () => {
  ...
  const actions = {
    ...
    openAssinanteModal: useCallback(
      (contract: Contract, template: string, documentType: string) => {
        dispatch({
          type: 'OPEN_ASSINANTE_MODAL',
          payload: { contract, template, documentType },
        });
      },
      []
    ),
    ...
    // Formulários
    updateAgendamentoData: useCallback(
      (data: Partial<ContractModalState['formData']['agendamento']>) => {
        dispatch({ type: 'UPDATE_AGENDAMENTO_DATA', payload: data });
      },
      []
    ),

    updateRecusaData: useCallback(
      (data: Partial<ContractModalState['formData']['recusaAssinatura']>) => {
        dispatch({ type: 'UPDATE_RECUSA_DATA', payload: data });
      },
      []
    ),

    updateWhatsAppData: useCallback(
      (data: Partial<ContractModalState['formData']['whatsApp']>) => {
        dispatch({ type: 'UPDATE_WHATSAPP_DATA', payload: data });
      },
      []
    ),

    updateAssinanteData: useCallback(
      (data: Partial<ContractModalState['formData']['assinante']>) => {
        dispatch({ type: 'UPDATE_ASSINANTE_DATA', payload: data });
      },
      []
    ),

    updateNPSData: useCallback(
      (data: Partial<ContractModalState['formData']['nps']>) => {
    ...
    // Reset
    resetModalData: useCallback(
      (modalType: keyof ContractModalState['formData']) => {
  ...
  return {
    ...
    isModalOpen: (modalType: keyof ContractModalState['modals']) =>
