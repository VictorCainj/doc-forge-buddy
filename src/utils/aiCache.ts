...
class AICache {
  ...
  private isSimilar(input1: string, input2: string, threshold: number = 0.8): boolean {
  ...
  get(input: string, mode: 'normal' | 'intelligent' | 'analysis'): CacheEntry | null {
  ...
  private fuzzySearch(input: string, mode: string): CacheEntry | null {
    ...
    if (bestMatch) {
      ...
      log.debug(`Cache fuzzy hit (${Math.round(bestSimilarity * 100)}%) para: ${input.substring(0, 50)}...`);
    }
  ...
  private evictLeastUsed(): void {
    ...
    entries.sort((a, b) => {
      ...
      const scoreA = a[1].usageCount - (Date.now() - a[1].lastUsed.getTime()) / (1000 * 60 * 60);
      const scoreB = b[1].usageCount - (Date.now() - b[1].lastUsed.getTime()) / (1000 * 60 * 60);
  ...
  private cleanup(): void {
    ...
    if (removedCount > 0) {
      log.debug(`Limpeza do cache: ${removedCount} entradas expiradas removidas`);
      this.saveToStorage();
    }
  }
